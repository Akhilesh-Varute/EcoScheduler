...
from common.db_models import DynamoDBTables, ScheduleModel, SavingsModel, AuditLogModel
...
def handler(event, context):
    """
    Handle stopping EC2 instances - can be triggered by:
...
def handle_api_request(event, context):
    """
    Handle API request to stop instances
    ...
    try:
        ...
        # Check permissions
        has_stop_permission = has_permission(user, "stop_instances")
        is_admin = user["role"] == "admin"
        has_account_access = account_id in user.get("awsAccounts", [])

        if not (is_admin or (has_stop_permission and has_account_access)):
            return create_response(
                403,
                {
                    "success": False,
                    "message": "You do not have permission to stop instances in this account",
                },
            )
        ...
        if dry_run:
            print(f"[DRY RUN] Would stop instances {instance_ids} in account {account_id}")
            ...
            return create_response(
                200,
                {
                    "success": True,
                    "dryRun": True,
                    "message": f"[Dry run] Would have stopped {len(instance_ids)} instance(s)",
                    "instances": instance_ids,
                },
            )

        # Initialize EC2 connector and stop instances
        ec2 = EC2Connector(account_id)
        result = ec2.stop_instances(instance_ids)
        ...
        print(f"Error in EC2 stop API handler: {str(e)}")
        return create_response(
...
def handle_schedule_event(event, context):
    """
    Handle EventBridge scheduled event to stop instances
    ...
    try:
        ...
        if schedule.get("dryRun", False):
            print(f"[DRY RUN] Schedule {schedule_id}: would stop instances {instance_ids}")
            audit_model.record_action(
                action="stop",
        ...
        # Record stop time for savings calculation
        current_time = datetime.utcnow().isoformat()
        today_date = datetime.utcnow().strftime("%Y-%m-%d")

        if result.get("success"):
            # Update schedule with latest stop time
            schedule_model.update_schedule(
                schedule_id,
                {
            ...
            # Calculate savings if instances were previously started
            last_start_time = schedule.get("lastStartTime")

            if last_start_time:
                try:
                    # Calculate hours between start and stop
                    start_time = datetime.fromisoformat(last_start_time)
                    stop_time = datetime.fromisoformat(current_time)

                    hours_saved = (stop_time - start_time).total_seconds() / 3600

                    # Calculate savings for each instance
                    for instance_id in instance_ids:
                        ...
                        if instance_response.get("success") and instance_response.get(
                            "instances"
                        ):
                            ...
                            savings = ec2.calculate_instance_savings(
                                instance_id, hours_saved
                            )
                            ...
                            savings_model.record_savings(
                                ...
                                cost_saved=savings.get("savings", 0),
                                date=today_date,
                            )
                except Exception as e:
        ...
        print(f"Error in EC2 stop schedule handler: {str(e)}")
